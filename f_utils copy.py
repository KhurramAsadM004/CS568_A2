...
def softmax(a):
    ...
    # if a_sum == 0 or np.isnan(a_sum):
    # print(a)
    # print("Before, SUM: ", a_sum)
    # eps = 1e-9    
    # a_sum[np.isnan(a_sum)] = eps
    # a_sum = np.where((a_sum == 0) or np.isnan(a_sum), eps, a_sum)
    
    # if not a_sum[np.isnan(a_sum)].size == 0:
    #     a_sum[np.isnan(a_sum)] = eps
    #     print("After, SUM: ", a_sum[np.isnan(a_sum)])

    z = np.divide(a_exp, a_sum) #softmax results
    # z = a_exp / a_exp.sum(axis=0, keepdims=True)
    return z
...
def mce_new(self, y, batch_target):
    loss = 0
    # y = softmax(y)
    
    for i in range(len(y)):
        loss = loss + (-1 * batch_target[i] * np.log(y[i]))
    return loss
...
def mce(self, y, batch_target):
    ...
    for i in y:
        ...
        y_loss = 0
        for j in i:
            y_loss += batch_target[m][n] * np.log(max(j, eps))
            n += 1 
        loss += y_loss
        m += 1
